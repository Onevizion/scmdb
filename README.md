...
* ```--gen-ddl``` generate DDL for objects created with new scripts, regenerate JSON schemas for changed tables and component structure files
* ```--gen-all-schemas``` regenerate JSON schemas for all table DDL files from sibling ```ddl``` directory into sibling ```json``` directory and component structure files into sibling ```json-component-structures``` directory
