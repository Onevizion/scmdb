...
* ```--gen-ddl``` generate DDL for objects created with new scripts, regenerate JSON schemas for changed tables and component structure files (requires Python 3.10+)
* ```--gen-all-schemas``` regenerate JSON schemas for configuration tables selected from V_COMPONENT/V_COMPONENT_TABLE using sibling ```ddl``` directory DDL files into sibling ```json``` directory, and component structure files into sibling ```json-component-structures``` directory (requires Python 3.10+)
...
DDL generation works by regexping new DB scripts for DB objects creation/modification and then extracting DDLs from the DB. JSON schema generation uses the same changed object list and writes schemas for affected tables. Component structure generation uses V_COMPONENT and V_COMPONENT_TABLE data.

To regenerate all JSON schemas without extracting DDL from the database:

```java -jar scmdb.jar --owner-schema=vqs_p01_epm/vepm@localhost:1521:orclpdb --scripts-dir=./db/scripts --gen-all-schemas```
